...
def segment_wm(feature_heatmap: sana.image.Frame, 
               tissue_mask: sana.image.Frame, 
               tb: sana.image.Frame, 
               logger: sana.logging.Logger=None, output_path: str=None, 
               debug_directory: str=None, **kwargs):
    if logger is None:
    ...
    w_heatmap, h_heatmap = feature_heatmap.size()
    logger.debug(f"Thumbnail: {tb.size()} / Feature heatmap: {feature_heatmap.size()}")

    # --- (1) GMM: GM/WM posteriors from the soma feature heatmaps ---
...
def run_gmm(tb: sana.image.Frame, tissue_mask: sana.image.Frame, 
            features: sana.image.Frame, logger=None, debug=False,
    ...
    features    : sana Frame (H, W, 3) feature heatmap -- soma density, size, intensity
    ...
    """
    if logger is None:
        logger = sana.logging.Logger('normal', fpath=None)
    ...
    gm_prob = np.zeros(tissue_mask_features.img.shape[:-1], dtype=float)
    gm_prob[tissue_mask_features.img[:,:,0] != 0] = probs[:, gm_label]
...
def post_process(gm_prob, tissue_mask, mpp, beta=8, connectivity=8, min_island_mm2=1.0,
                 logger=None, output_directory=None):
    ...
    tissue = tissue_mask.img[:,:,0].astype(bool)
...
def viz_gmm_result(tb, tissue_mask_features, soma_density, soma_size, inliers,
                   X_scaled_tissue, labels, gmm, gm_label,
                   logger=None, n_scatter=20000, output_directory=None):
    ...
    label_map = np.zeros(tissue_mask_features.img.shape[:-1], dtype=np.uint8)  # 0 none, 1 GM, 2 WM
    label_map[tissue_mask_features.img[:,:,0] != 0] = np.where(labels == gm_label, 1, 2)
    dropped_map = np.zeros(tissue_mask_features.img.shape[:-1], dtype=bool)
    dropped_map[tissue_mask_features.img[:,:,0] != 0] = ~inliers
    ...
    d_tissue, s_tissue = soma_density[tissue_mask_features.img[:,:,0] != 0], soma_size[tissue_mask_features.img[:,:,0] != 0]
    ...
    def feature_panel(ax, fmap, lo, hi, cmap, title):
        ...
        m = np.ma.masked_where(~(tissue_mask_features.img[:,:,0] != 0), fmap)
        ...
        ax.legend(handles=[Patch(facecolor=DROPPED_C,
                                 label=f'dropped  {100*dropped_map.sum()/tissue_mask_features.img[:,:,0].sum():.1f}%')],
